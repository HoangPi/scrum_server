    ...
    : m_authHandler(jwt)
{
    ...
    authEndpoints.route("POST", "users/refresh", false);
    authEndpoints.route("OPTIONS", "*", false);
    authEndpoints.route("GET", "/", false);
    authEndpoints.route("GET", "/users/signout", false);
...
std::shared_ptr<AuthInterceptor::OutgoingResponse> AuthInterceptor::intercept(const std::shared_ptr<IncomingRequest> &request)
{
    ...
    auto authObject = std::static_pointer_cast<JWT::Payload>(m_authHandler.authorize(authHeader));
    if (authObject)
    {
